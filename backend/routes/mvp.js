// AURA MVP - Endpoints pour Frontend
...
router.get('/ai/observability/summary', (req, res) => {
  // Simulate real-time variations
  const metrics = {
    ...mockMetrics,
    tokens_saved_ratio: mockMetrics.tokens_saved_ratio + (Math.random() - 0.5) * 0.1,
    cache_hit_ratio: mockMetrics.cache_hit_ratio + (Math.random() - 0.5) * 0.05,
    rag_p95: mockMetrics.rag_p95 + Math.floor((Math.random() - 0.5) * 100),
    router_bypass_rate: mockMetrics.router_bypass_rate + (Math.random() - 0.5) * 0.05
  };
...
// Router decisions
router.get('/ai/router/decisions', (req, res) => {
...
<body>
  ...
  <div class="content">
    ...
    <p>Build time: ${artifact.build_ms}ms</p>
    <p>Content hash: <code>${artifact.content_hash}</code></p>
    <h2>Sample Analysis</h2>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. This document contains 
    <span class="entity">sample entities</span> and <span class="entity">mock data</span> 
...
// Artifact metadata
router.get('/artifacts/:id/meta.json', (req, res) => {
  const { id } = req.params;
  const artifact = mockArtifacts.find(a => a.id === id);
...
router.get('/ai/stream/metrics', (req, res) => {
  res.writeHead(200, {
