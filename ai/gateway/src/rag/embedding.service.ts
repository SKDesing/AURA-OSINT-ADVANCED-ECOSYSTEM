...
type EmbedProvider = 'local' | 'openai';

export class EmbeddingService {
  private static instance: EmbeddingService;
  private modelName = process.env.EMB_MODEL || 'Xenova/multilingual-e5-small';
  private provider: EmbedProvider = (process.env.EMB_PROVIDER as EmbedProvider) || 'local';
  private cacheDir = process.env.EMB_CACHE_DIR || '.cache/embeddings';
  ...
  private promptHash(text: string): string {
  ...
  async healthCheck(): Promise<{ ok: boolean; provider: string; model: string; dimensions?: number }> {
    try {
  ...
  getCacheStats(): { size: number; totalFiles: number } {
    try {
